...
- Total: **377 tests** across **48 files** (all plain `#[test]`; no integration `tests/` crate).
...
| --- | --- | --- |
| [`mod.rs`](../../src-tauri/src/catalog/mod.rs) | 74 | Catalog assembly, `sanitize`/`sanitize_reported`, phantom-drop (`target_is_present`), console demotion, portable naming, and the `classify`/`classify_app` category cases (Steam/publisher/install-path/resolved-exe/product-name signals, new categories, false-positive fixes, locale-independent classification). |
| [`dedup/mod.rs`](../../src-tauri/src/catalog/dedup/mod.rs) | 63 | Evidence-based merging, `should_merge` thresholds, publisher/version/install-root vetoes, deterministic `canonical_order_key` + fixed-point loop, the differential harness vs a reference resolver and the scale invariant, system-tool aliases (File Explorer, Run dialog, …), sticky-auxiliary survival through a merge, and the locale-aware display-name pick (`name_script`/`choose_display_name`). |
| [`visibility/mod.rs`](../../src-tauri/src/catalog/visibility/mod.rs) | 32 | `classify_visibility` scoring and Primary/Auxiliary/Rejected reason codes: installer artifacts vs runtime redistributables and the launch-registration veto, framework packages, shell-location shortcuts, maintenance/console/command-environment, product components, architecture stubs, bundled toolchains, dead AutoGenerated Start-Apps, console demotion surviving reclassification, and the synthetic fixture corpus. |
...
| [`storage/cache.rs`](../../src-tauri/src/catalog/storage/cache.rs) | 12 | `apps-cache.json` load/store: exact-version load, older-version upgrade, pre-schema array, atomic write + `.bak` recovery, reset leaving preferences intact. |
...
| [`sources/start_apps.rs`](../../src-tauri/src/catalog/sources/start_apps.rs) | 13 | Start Menu / AUMID discovery and target resolution, plus the bounded, killable PowerShell helper (timeout, cancellation, output cap). |
...
The `dedup/` submodules (`evidence`, `signals`, `family`, `target`, `identity`, `blocking`, `merge`,
...
| [`execution/executable_metadata.rs`](../../src-tauri/src/platform/windows/execution/executable_metadata.rs) | 7 | PE version-resource reads: ProductName/OriginalFilename, bounded allocation, malformed-PE safety. |
...
- Frontend tests are separate: `tests/frontend/**` (Vitest, 289 tests across 41 files), run with
