...
pub(super) fn scan(hive: winreg::HKEY, subkey: &str) -> Vec<AppInfo> {
    let Ok(uninstall) = RegKey::predef(hive).open_subkey(subkey) else { return Vec::new() };
    uninstall.enum_keys().filter_map(Result::ok)
        .filter_map(|name| uninstall.open_subkey(name).ok())
        .filter_map(|key| from_values(RegistryValues {
            display_name: key.get_value("DisplayName").ok()?,
...
pub(super) fn from_values(values: RegistryValues) -> Option<AppInfo> {
    ...
    let uninstall = values.quiet_uninstall_string.as_deref().and_then(split_command)
