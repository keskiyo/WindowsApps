...
pub(super) enum Validated {
    Process { program: PathBuf, args: Vec<String> },
    Msix { package_full_name: String },
}

pub(super) fn validate(target: &UninstallTarget) -> Result<Validated, String> {
    match target {
...
pub(super) fn is_msiexec(executable: &str) -> bool {
...
mod tests {
    use super::*;
    use crate::platform::windows::uninstall::uninstaller::tests::{command, msix};
