...
use markers::{
    executable_matches_product, has_launch_registration, has_runtime_path,
    is_architecture_stub_executable, is_bundled_toolchain_path, is_command_environment,
    is_dead_autogenerated_startapp, is_documentation, is_framework_package, is_installer_artifact,
    is_maintenance_executable, is_product_component, is_runtime_redistributable, is_sdk_sample,
    is_shell_location_shortcut,
};
...
pub(crate) fn classify_visibility(app: &AppInfo) -> VisibilityDecision {
    ...
    if is_runtime_redistributable(&value) {
    ...
    // The installer and documentation markers are plain substrings of a combined blob, so they fire
    // on real applications whose name merely reads like an installer ("Inno Setup", "Advanced
    // Installer"). Rejection is unrecoverable — rejected entries are dropped before deduplication
    // and no view exposes them — so registration evidence outranks a name that only looks wrong.
    let registered = has_launch_registration(app);
    if !registered && is_installer_artifact(&value) {
        return VisibilityDecision {
            class: VisibilityClass::Rejected,
            score: -100,
            reasons: vec![VisibilityReason::Installer],
    ...
    if !registered && is_documentation(&value) {
        return VisibilityDecision {
            class: VisibilityClass::Rejected,
            score: -80,
