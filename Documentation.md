...
Frontend preferences use schema version 14. Each launch card has a `preferenceIdentity`
...
scenarios, version 12 their creation date, which orders the More preview, version 13 the
starred scenarios, and version 14 bounded scenario-run history; an earlier document upgrades to an empty or undated value and keeps every other
