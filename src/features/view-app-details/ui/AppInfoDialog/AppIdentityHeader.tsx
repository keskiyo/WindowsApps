...
export function AppIdentityHeader({
	...
	return (
		<header className='flex items-start gap-4 sm:gap-5'>
			<span className='grid size-17 shrink-0 place-items-center overflow-hidden rounded-2xl border border-[var(--border-neutral)] bg-[var(--surface-raised)] shadow-[var(--shadow-accent-soft)] sm:size-20'>
				{app.iconBase64 ? (
			...
			<div className='min-w-0 flex-1 pt-0.5'>
				<div className='flex flex-wrap items-center gap-x-2 gap-y-1'>
					<h2 className='truncate text-xl font-semibold tracking-tight text-[var(--text-primary)] sm:text-2xl'>
						{app.name}
					</h2>
				...
				<div className='mt-3 flex flex-wrap gap-2 text-sm'>
					{app.version && (
						<span className='rounded-lg border border-[var(--border-neutral)] bg-[var(--surface-inset)] px-2.5 py-1 text-[var(--text-muted)]'>
							v{displayVersion(app.version)}
						</span>
					)}
					<span className='rounded-lg border border-[var(--border-neutral)] bg-[color-mix(in_oklab,var(--accent)_15%,var(--surface-raised))] px-2.5 py-1 text-[var(--accent-strong)]'>
