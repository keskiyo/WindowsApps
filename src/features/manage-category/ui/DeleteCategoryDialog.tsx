	...
	return (
		<div className="motion-overlay fixed inset-0 z-400 grid place-items-center bg-slate-700/38 p-4 backdrop-blur-[2px]">
			<section
				...
				className="motion-panel w-full max-w-md rounded-2xl border border-red-300/55 bg-slate-50 p-5 text-slate-800 shadow-(--shadow-dialog)"
			>
				<div className="flex items-start gap-3">
					<Trash2 className="mt-0.5" size={20} />
				...
				<div className="mt-5 flex justify-end gap-3">
					...
					<button
						type="button"
						onClick={onConfirm}
						className={`rounded-xl px-4 py-2 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 ${DANGER_VARIANT}`}
					>
						Delete category
					</button>
				</div>
