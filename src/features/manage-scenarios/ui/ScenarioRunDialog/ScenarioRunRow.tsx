...
interface Props {
	scenario: Scenario
	/** The catalog the scenario's stored identities resolve against. */
	apps: AppInfo[]
	expanded: boolean
	running: boolean
...
/**
 * One scenario in the run dialog: collapsed to its name and list sizes, expanded to the apps it
 * actually holds. Collapsed is the default because the point of this dialog is picking a scenario
 * to run; the contents are what you open when you are not sure which one it is.
 */
export function ScenarioRunRow({
	...
	return (
		<li className='border-t border-(--border-neutral) first:border-t-0'>
			...
			{expanded && (
				<div id={panelId} className='flex flex-col gap-3 px-4 pt-1 pb-3'>
					<ScenarioRunList
						label='Launch'
