...
describe('app store', () => {
	...
	it('hides and restores an app without losing its category or favorite', () => {
		...
		expect(selectVisibleApps(store.getState()).map(app => app.id)).not.toContain('code')
		store.getState().setActiveView('hidden')
		expect(selectVisibleApps(store.getState()).map(app => app.id)).toEqual(['code'])
	...
	it('launches and uninstalls through source-aware client calls', async () => {
		...
		await expect(store.getState().uninstall('codex')).resolves.toBeUndefined()
