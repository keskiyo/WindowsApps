		...
		<section className='grid min-h-[55vh] place-items-center px-4 text-center'>
			<div className='relative w-full max-w-md rounded-2xl border border-white/85 bg-white/58 px-8 py-10 shadow-(--shadow-soft-panel) backdrop-blur-xl'>
				<button
					...
					onClick={onDismiss}
					className='absolute right-3 top-3 grid size-9 place-items-center rounded-lg text-slate-500 hover:bg-violet-100/75 hover:text-slate-800 focus-visible:outline-2 focus-visible:outline-violet-500'
				>
				...
				<button
					...
					disabled={isScanning}
					onClick={() => void onScan()}
					className='utility-accent-button mt-6 rounded-xl px-5 py-2.5 text-sm font-semibold text-white shadow-(--shadow-accent) focus-visible:outline-2 focus-visible:outline-violet-500 disabled:opacity-60'
				>
