...
function renderApp(overrides: Partial<AppsClient> = {}) {
...
describe('App', () => {
	...
	it('hides an app and restores it from the Hidden view', async () => {
		...
		await userEvent.click(await screen.findByRole('button', { name: 'Manage Steam' }))
		await userEvent.click(screen.getByRole('menuitem', { name: 'Hide from catalog' }))
		expect(screen.queryByRole('button', { name: 'Launch Steam' })).not.toBeInTheDocument()
		...
		expect(await screen.findByRole('button', { name: 'Launch Steam' })).toBeInTheDocument()
		await userEvent.click(screen.getByRole('button', { name: 'Manage Steam' }))
		await userEvent.click(screen.getByRole('menuitem', { name: 'Restore to catalog' }))
	...
	it('disables uninstall when Windows has no registered uninstall target', async () => {
		const { client } = renderApp()
		await userEvent.click(
